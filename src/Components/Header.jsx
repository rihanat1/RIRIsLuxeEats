...
const Header = () => {
  ...
  return (
    <div className="flex justify-center h-20 bg-black/50 border-b border-[#4D4635] shadow-sm z-50 backdrop-blur-lg fixed top-0 right-0 left-0">
      <div className="flex justify-between items-center w-[95%] xl:w-[85%] m-auto md:flex md:justify-between">
  ...
  `}
        >
          {navItems.map((item) => (
            <li
              className={`
                text-[#555F6F] border-b-2 p-4 font-semibold border-primary 
                w-full text-center text-[16px] list-none
                md:border-none md:w-auto md:p-0 md:font-light whitespace-nowrap
              `}
              key={item.label}
            >
        ...
        <div className="z-50 flex gap-4 items-center justify-center">
          ...
          <div onClick={handleSubMenu} className="cursor-pointer md:hidden">
            {subMenu ? (
              <IoMdClose className="mt-[4px] z-50 text-3xl text-primary" />
            ) : (
              <RxHamburgerMenu className="mt-[4px] z-60 text-3xl text-primary" />
            )}
          </div>
        </div>
      </div>
