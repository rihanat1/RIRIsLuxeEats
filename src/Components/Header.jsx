...
const Header = () => {
  ...
  return (
    <div className="flex justify-center h-20 bg-white/80 shadow-sm z-50 backdrop-blur-sm fixed top-0 right-0 left-0">
      <div className="flex justify-between items-center w-[95%] m-auto md:flex md:justify-between border-2 border-green-500 xl:w-[85%]">
        <div className="flex items-center space-x-2 z-50">
          <GiKnifeFork className="text-[#9D4300] text-[30px]" />
          <p className="z-50 text-[#9D4300] font-bold text-[25px]">
            RIRI's LuxeEats
          </p>
        </div>
        ...
        bg-white/80 shadow-sm z-30 backdrop-blur-sm left-0 right-0  pb-4  md:static md:flex-row md:gap-8 md:items-center md:h-fit md:w-[39%] md:justify-center md:pb-0 md:bg-transparent md:shadow-none md:backdrop-blur-none lg:w-[30%]
    ${
      subMenu
  ...
  `}
        >
          {navItems.map((item) => (
            <li
              className={`
                text-[#555F6F] border-b-2 p-4 font-semibold border-[#9D4300] 
        ...
        <div className="z-50 flex gap-4 items-center">
          <NavLink to="/cart">
            <div className="z-50 flex items-center gap-2 rounded-xl text-[14px] font-bold bg-[#9D4300] text-white px-3 py-1 hover:bg-[#7a3500] transition">
              <FaOpencart />
              <span>Cart (0)</span>
            </div>
          </NavLink>
          <div onClick={handleSubMenu} className="cursor-pointer md:hidden">
            {subMenu ? (
              <IoMdClose className="mt-[6px] z-50 text-3xl text-[#9D4300]" />
            ) : (
              <RxHamburgerMenu className="mt-[6px] z-50 text-3xl text-[#9D4300]" />
            )}
          </div>
        </div>
      </div>
